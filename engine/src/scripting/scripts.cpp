...
namespace tbx
{
    ...
    void internal::fixed_update_scripts(ScriptsState& state, Sandbox& sandbox, const float fixed_delta_time)
    {
        sandbox.for_each_with<Script>(
            [&](Toy toy, Script& script)
            {
                ...
                internal::ensure_started(state, toy, script.source.id); // no-op once started
                for (const auto& backend : state.backends)
    ...
    {
        const auto backend = internal::route(state, name);
        if (!backend)
        ...
        const float delta_time)
    {
        ...
        // frame (present), and start-once + update the enabled ones. internal::ensure_started only calls
        ...
        sandbox.for_each_with<Script>(
            [&](Toy toy, Script& script)
            {
                ...
                if (!toy.is_enabled())
                    return;
                internal::ensure_started(state, toy, script.source.id);
                for (const auto& backend : state.backends)
        ...
        for (auto it = state.live_scripts.begin(); it != state.live_scripts.end();)
        {
            ...
            internal::cleanup_and_purge(state, sandbox, it->first, it->second);
            it = state.live_scripts.erase(it);
        }
    }
    ...
    void internal::purge_scripts(ScriptsState& state, Sandbox& sandbox)
    {
        for (const auto& [id, source_id] : state.live_scripts)
            internal::cleanup_and_purge(state, sandbox, id, source_id);
