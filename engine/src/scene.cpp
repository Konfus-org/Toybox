...
// tbx::internal::current() (main thread only).
namespace tbx
{
    Toy spawn(std::string name)
    {
        return internal::current().sandbox.add(std::move(name));
    }

    std::optional<Toy> find(std::string name)
    {
        return internal::current().sandbox.find(std::string_view(name));
    }

    void despawn(Toy toy)
    {
        internal::current().sandbox.remove(toy);
    }

    std::vector<Toy> toys()
    {
        return internal::current().sandbox.get_toys();
    }

    bool is_key_down(Key key)
    {
        return is_down(internal::current().input, key);
    }

    bool is_key_pressed(Key key)
    {
        return is_pressed(internal::current().input, key);
    }

    bool is_key_released(Key key)
    {
        return is_released(internal::current().input, key);
    }

    bool is_mouse_down(MouseButton button)
    {
        return is_down(internal::current().input, button);
    }

    bool is_mouse_pressed(MouseButton button)
    {
        return is_pressed(internal::current().input, button);
    }

    bool is_mouse_released(MouseButton button)
    {
        return is_released(internal::current().input, button);
    }

    float get_mouse_axis(MouseAxis axis)
    {
        return get_axis(internal::current().input, axis);
    }

    std::optional<RaycastHit> raycast(const Vec3& origin, const Vec3& direction, float max_distance)
    {
        return internal::raycast(internal::current().physics, origin, direction, max_distance);
    }
