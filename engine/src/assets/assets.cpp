...
namespace tbx
{
    ...
    void internal::update_assets(AssetsState& state, EventsState& events)
    {
        ...
        auto unloaded = std::vector<AssetReloaded>(); // reuse the id+extension shape
        {
            ...
            for (auto it = state.loaded_assets.begin(); it != state.loaded_assets.end();)
            {
                ...
                const auto relative = internal::find_path_of_locked(state, it->first);
                unloaded.push_back(internal::make_reloaded_event(it->first, relative.value_or("")));
    ...
    void internal::purge_assets(AssetsState& state, EventsState& events)
    {
        ...
        auto unloaded = std::vector<AssetReloaded>(); // reuse the id+extension shape
        {
            ...
            for (const auto& entry : state.loaded_assets)
                unloaded.push_back(internal::make_reloaded_event(
                    entry.first,
                    internal::find_path_of_locked(state, entry.first).value_or("")));
        ...
        const std::string& path)
    {
        if (id.is_valid())
        {
            ...
            const auto tracked = internal::find_relative_path(state, id);
        ...
        const auto prepared = internal::prepare(state, path);
        if (!prepared)
        ...
        std::filesystem::path root)
    {
        ...
        internal::discover_assets(state);
    }
        ...
        std::filesystem::path root)
    {
        state.root = std::move(root);
        {
            ...
            const std::scoped_lock lock(state.mutex);
            internal::index_meta_sidecars(state);
        }
        ...
        state.watcher.emplace(
            ...
            [&state, &events, &jobs](const std::filesystem::path& path)
            {
                post_main(
                    ...
                    [&state, &events, path]
                    {
                        internal::handle_file_changed(state, events, path);
                    });
            });
    }
        ...
        std::any asset)
    {
        ...
        const AssetReloaded stamped = internal::make_reloaded_event(id, relative_path);
