...
namespace tbx
{
    ...
    std::vector<float> build_cube_vertices()
    {
        ...
        internal::push_box(vertices, Vec3(0.0f), Vec3(0.5f, 0.5f, 0.5f));
        return vertices;
    }
    ...
    std::vector<float> build_fullscreen_vertices()
    {
        ...
        internal::push_vertex(vertices, Vec3(-1.0f, -1.0f, 0.0f), forward, Vec2(0.0f, 0.0f));
        internal::push_vertex(vertices, Vec3(3.0f, -1.0f, 0.0f), forward, Vec2(2.0f, 0.0f));
        internal::push_vertex(vertices, Vec3(-1.0f, 3.0f, 0.0f), forward, Vec2(0.0f, 2.0f));
        return vertices;
    }
    ...
    std::vector<float> build_plane_vertices()
    {
        ...
        const Vec2 uvs[4] = {{0, 0}, {1, 0}, {1, 1}, {0, 1}};
        for (const int index : {0, 1, 2, 0, 2, 3})
            internal::push_vertex(vertices, corners[index], up, uvs[index]);
        return vertices;
    }
    ...
    std::vector<float> build_question_mark_vertices()
    {
        ...
        internal::push_box(vertices, Vec3(0.0f, 0.42f, 0.0f), Vec3(0.22f, 0.07f, DEPTH)); // top of the arc
        internal::push_box(vertices, Vec3(-0.22f, 0.31f, 0.0f), Vec3(0.07f, 0.09f, DEPTH)); // arc start
        internal::push_box(vertices, Vec3(0.22f, 0.25f, 0.0f), Vec3(0.07f, 0.13f, DEPTH)); // arc right side
        internal::push_box(vertices, Vec3(0.07f, 0.12f, 0.0f), Vec3(0.16f, 0.06f, DEPTH)); // hook inward
        internal::push_box(vertices, Vec3(0.0f, -0.06f, 0.0f), Vec3(0.07f, 0.13f, DEPTH)); // stem
        internal::push_box(vertices, Vec3(0.0f, -0.40f, 0.0f), Vec3(0.09f, 0.09f, DEPTH)); // the dot
        return vertices;
    }
    ...
    std::vector<float> build_sphere_vertices(const int rings, const int segments)
    {
        ...
        auto point = [&](const int ring, const int segment)
        {
            ...
            internal::push_vertex(vertices, normal * 0.5f, normal, uv);
        };
