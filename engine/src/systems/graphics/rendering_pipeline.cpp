...
namespace tbx
{
    ...
    const auto SCENE_CULLING_SHADER_HANDLE = Handle("Shaders/Pipeline/SceneCulling.comp");
    ...
    enum class ResourceCacheKey : uint64
    {
        ...
        CLUSTER_GRID,
        LIGHT_INDEX_POOL,
        MAIN_SCENE_DRAW_ARGS,
        SHADOW_ARGS_POOL,
        MAIN_SCENE_DRAW_COUNT,
        SHADOW_DRAW_COUNT,
        SCENE_UNIFORMS,
        GBUFFER_ALBEDO,
        GBUFFER_ROUGHNESS,
    ...
    struct FrameBuffers
    {
        ...
        GpuId scene_uniforms = INVALID_GPU_ID;
    ...
    struct UploadedFrameResources
    {
        ...
        std::vector<SceneRasterBatch> scene_batches = {};
    ...
    struct RenderingPipeline::State final
    {
        ...
        SceneBuildResources frame_scene = {};
        std::vector<PendingSceneInstance> pending_instances = {};
    };
    ...
    static Result warn_backend_failure(std::string_view context, Result result)
    {
        if (result.succeeded())
            return result;
        ...
        {
            TBX_TRACE_WARNING("Rendering pipeline: {}.", context);
            return result;
        }

        TBX_TRACE_WARNING("Rendering pipeline: {}. {}", context, result.get_report());
        return result;
    }
    ...
    static void reset_scene_build_resources(SceneBuildResources& scene)
    {
        scene.lights.clear();
        scene.instances.clear();
        scene.dynamic_mesh_sources.clear();
        scene.static_mesh_sources.clear();
        scene.view_projection = Mat4(1.0F);
        scene.camera_position = Vec3(0.0F);
        scene.sky_color = Vec4(0.55F, 0.72F, 0.9F, 1.0F);
    }
    ...
    static uint64 hash_scene_pipeline_key(const ShaderProgram& shader, const MaterialConfig& config)
    {
    ...
    static Result validate_scene_raster_shader(const Material& material)
    {
        if (!material.shader.is_valid())
        {
            return make_failure(
                "Rendering pipeline: scene material is missing a valid raster shader program.");
        }
        return Result(true);
    }
        ...
        const GpuId gpu_id = INVALID_GPU_ID)
    {
        ...
        if (record == nullptr)
            return make_failure("Rendering pipeline: missing cached texture record.");
        ...
        const FrameBuffers& buffers)
    {
        ...
        append_binding(bindings, SHADER_BINDING_SCENE_DEPTH, buffers.scene_depth);
        ...
        const GpuId material_id)
    {
        ...
        if (material_id >= materials.size())
            return make_failure("Rendering pipeline: material upload index out of range.");
        ...
        if (needs_full_upload)
        {
            if (auto result =
                ...
                !result)
            {
                return warn_backend_failure("failed to upload global material buffer", result);
            }
        }
        else
        {
    ...
    static Result upload_frame_resources(
        ...
        auto& state,
        AssetManager& assets,
        const SceneBuildResources& scene,
        ...
        UploadedFrameResources& resources)
    {
        ...
        if (
            ...
            // the scene geometry is static, which dominated steady-state frame work.
            geometry_changed || !has_cached_geometry)
        {
            if (auto result = rebuild_geometry_buffers(backend, state, assets, scene, buffers);
                !result)
            {
                return result;
            }
        ...
        if (auto result = create_or_update_buffer(
                backend,
                state,
                cache_key(ResourceCacheKey::MAIN_SCENE_DRAW_ARGS),
                "main_scene_draw_args",
                draw_usage,
                byte_size(resources.max_scene_draw_count, sizeof(ShaderDrawIndexedIndirectCommand)),
                &empty_command,
        ...
        if (auto result = create_or_update_buffer(
                backend,
                state,
                cache_key(ResourceCacheKey::MAIN_SCENE_DRAW_COUNT),
                "main_scene_draw_count",
                draw_usage,
        ...
        if (auto result = create_or_update_texture(
                backend,
                state,
                cache_key(ResourceCacheKey::SCENE_DEPTH),
                depth_texture,
                "scene_depth",
                1U,
                false,
    ...
    static void apply_sky_background_color(
        ...
        auto& state,
        const Sky& sky,
        SceneBuildResources& scene)
    {
        static_cast<void>(assets.load<Material>(FALLBACK_MATERIAL_HANDLE));
        ...
        static_cast<void>(assets.load<Model>(FALLBACK_MESH_HANDLE));
        if (!material)
            return;
        ...
        {
            scene.sky_color = *background;
            return;
        }

        scene.sky_color = Vec4(1.0F, 0.0F, 1.0F, 1.0F);
    }
        ...
        const Mesh*& out_mesh)
    {
        ...
        if (entity.has_component<DynamicMesh>())
        {
            ...
            scene.dynamic_mesh_sources[out_mesh_id] = mesh_data;
        ...
        scene.static_mesh_sources[out_mesh_id] = static_mesh.handle;
        ...
        FrameBuffers& buffers)
    {
        ...
        for (auto& record : mesh_records)
        {
            ...
            if (const auto dynamic_source = scene.dynamic_mesh_sources.find(record.resource);
                dynamic_source != scene.dynamic_mesh_sources.end())
            {
                const auto dynamic_mesh = dynamic_source->second.lock();
                mesh = dynamic_mesh ? &dynamic_mesh->get_mesh() : nullptr;
            }
            else if (
                const auto static_source = scene.static_mesh_sources.find(record.resource);
                static_source != scene.static_mesh_sources.end())
            {
    ...
    static void resolve_scene_camera(
        World& world,
    ...
    static Result extract_scene_resources(
        IGraphicsBackend& backend,
        auto& state,
        AssetManager& assets,
        World& world,
        ...
        bool& out_geometry_changed)
    {
        ...
        collect_lights(world, scene.lights, resources.shadow_softness, shadow_layer_count);
        ...
        for (const auto& entity : world.get_with<Transform, MaterialInstance>())
        {
            ...
            if (auto result = validate_scene_raster_shader(*material); !result)
            {
                return result;
            }
            ...
            pending_instances.push_back(
                PendingSceneInstance {
                    ...
                    .pipeline_key =
                        hash_scene_pipeline_key(material->shader, resolved_material_config),
                    .mesh_id = static_cast<uint32>(mesh_id),
                    .material_id = static_cast<uint32>(material_id),
        ...
        std::sort(
            ...
            [](const PendingSceneInstance& lhs, const PendingSceneInstance& rhs)
            {
        ...
        scene.instances.reserve(pending_instances.size());
        resources.scene_batches.clear();
        for (const auto& pending : pending_instances)
        {
            const auto first_instance = static_cast<uint32>(scene.instances.size());
            scene.instances.push_back(pending.instance);

            if ((pending.pipeline_flags & SHADER_PIPELINE_FLAG_OPAQUE) == 0U)
                continue;

            const bool starts_new_batch =
                resources.scene_batches.empty()
                || resources.scene_batches.back().pipeline_key != pending.pipeline_key
                || resources.scene_batches.back().shader_key != pending.shader_key
                || resources.scene_batches.back().material_id != pending.material_id
                || resources.scene_batches.back().mesh_id != pending.mesh_id;
            if (starts_new_batch)
            {
                resources.scene_batches.push_back(
                    SceneRasterBatch {
            ...
            {
                resources.scene_batches.back().instance_count += 1U;
            }
        }

        if (const auto sky_entity = world.first_with<Sky, Transform>();
            sky_entity.get_id().is_valid())
            apply_sky_background_color(assets, state, sky_entity.get_component<Sky>(), scene);

        resources.instance_count = static_cast<uint32>(scene.instances.size());
        resources.shadow_count = shadow_layer_count;
        resources.max_scene_draw_count = resources.instance_count;
        resources.max_shadow_draw_count = resources.instance_count * resources.shadow_count;
        if (resources.instance_count == 0U)
            return Result(true);

        const auto scene_uniforms = ShaderSceneUniforms {
            .view_projection = scene.view_projection,
            .inverse_view_projection = glm::inverse(scene.view_projection),
            .frustum_planes = extract_frustum_planes(scene.view_projection),
            .ambient_light = Vec4(0.04F, 0.04F, 0.04F, 1.0F),
            .camera_position_time = Vec4(scene.camera_position, elapsed_time),
            ...
            .light_count = static_cast<uint32>(scene.lights.size()),
            .shadow_count = resources.shadow_count,
            .max_scene_draw_count = resources.max_scene_draw_count,
        ...
        UploadedFrameResources& resources)
    {
        ...
        auto& scene = state.frame_scene;
        reset_scene_build_resources(scene);
        ...
        return extract_scene_resources(
            backend,
            state,
            assets,
            world,
    ...
    static Result run_compute_submit(
        IGraphicsBackend& backend,
        auto& state,
        AssetManager& assets,
        const UploadedFrameResources& resources)
    {
        ...
        auto scene_culling = INVALID_GPU_ID;
        ...
        if (auto result = get_compute_pipeline(
                ...
                state,
                assets,
                hash(std::string_view("scene_culling")),
                "scene_culling",
                SCENE_CULLING_SHADER_HANDLE,
                scene_culling);
            !result)
        {
            return result;
        ...
        if (auto result =
            ...
            !result)
        {
            return warn_backend_failure("failed to reset main draw count buffer", result);
        }
        if (auto result =
                backend
            ...
            !result)
        {
            return warn_backend_failure("failed to reset shadow draw count buffer", result);
        }
        ...
        return with_compute_pass(
            ...
            [&]() -> Result
            {
                ...
                if (auto result = backend.bind_compute_pipeline(scene_culling); !result)
                    return result;
    ...
    static Result render_gbuffer(
        ...
        const UploadedFrameResources& resources)
    {
        if (resources.scene_batches.empty())
            return Result(true);
        ...
        return with_render_pass(
            ...
            RenderPassDesc {
                ...
                .depth_stencil_target = buffers.scene_depth,
            ...
            [&]() -> Result
            {
                ...
                for (const auto& batch : resources.scene_batches)
                {
    ...
    static Result barrier_gbuffer_to_compute(IGraphicsBackend& backend, const FrameBuffers& buffers)
    {
        return backend.pipeline_barrier({
            ...
            PipelineBarrierDesc {
                .resource_handle = buffers.scene_depth,
                .state_before = ResourceState::DEPTH_WRITE,
                .state_after = ResourceState::SHADER_READ_ONLY,
            },
            PipelineBarrierDesc {
        ...
        const DeltaTime& delta_time)
    {
        ...
        const auto fail_frame = [&backend](const Result& result) -> Result
        {
            const auto failure_result = present_failure_frame(backend);
        ...
        if (auto result = upload_entities(
                backend,
                *_state,
                *asset_manager,
                *world,
        ...
        if (auto result = run_compute_submit(backend, *_state, *asset_manager, resources); !result)
            return fail_frame(result);

        if (auto result = barrier_compute_to_graphics(backend, resources.buffers); !result)
            return fail_frame(result);

        if (auto result = render_shadow_atlas(backend, *_state, *asset_manager, resources); !result)
            return fail_frame(result);

        if (auto result = barrier_shadow_to_gbuffer(backend, resources.buffers); !result)
            return fail_frame(result);

        if (auto result = render_gbuffer(backend, *_state, *asset_manager, resources); !result)
            return fail_frame(result);

        if (auto result = barrier_gbuffer_to_compute(backend, resources.buffers); !result)
            return fail_frame(result);

        if (auto result = resolve_lighting(backend, *_state, *asset_manager, resources); !result)
            return fail_frame(result);

        if (auto result = barrier_final_to_present(backend, resources.buffers); !result)
            return fail_frame(result);

        if (auto result = present_frame_texture(backend, *_state, *asset_manager, resources);
            !result)
            return fail_frame(result);
