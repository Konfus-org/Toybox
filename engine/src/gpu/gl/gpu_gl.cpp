...
namespace tbx
{
    ...
    void gpu_begin_frame(const FrameDescription& description)
    {
    ...
    void gpu_begin_render_pass(const RenderPassDescription& description)
    {
    ...
    Result<std::unique_ptr<Shader>> gpu_compile_shader(
    ...
    std::unique_ptr<Pipeline> gpu_make_pipeline(const PipelineDescription& description)
    {
    ...
    void gpu_set_scissor(const bool is_enabled, const int x, const int y, const int width, const int height)
    {
        if (!is_enabled)
        {
