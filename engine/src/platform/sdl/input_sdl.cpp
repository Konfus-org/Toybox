...
namespace tbx
{
    ...
    void internal::update_input(InputState& input, EventsState& events)
    {
        ...
        internal::roll_input_frame(input);
        ...
        // in right here rather than trusting the ADDED events alone — internal::open_gamepad de-dupes, so
        // a pad reported by both paths is claimed once.
        if (!internal::g_gamepad_subsystem_ready)
        {
            if (SDL_InitSubSystem(SDL_INIT_GAMEPAD))
            {
                internal::g_gamepad_subsystem_ready = true;
                int gamepad_count = 0;
                if (SDL_JoystickID* ids = SDL_GetGamepads(&gamepad_count))
                {
                    for (int i = 0; i < gamepad_count; ++i)
                        internal::open_gamepad(input, events, ids[i]);
                    SDL_free(ids);
                }
            }
            else
        ...
        while (SDL_PollEvent(&event))
        {
            switch (event.type)
            {
                ...
                {
                    const Key key = internal::translate_key(event.key.scancode);
                    if (key == Key::UNKNOWN)
                        break;
                    if (!event.key.repeat)
                        internal::feed_key(input, key, event.key.down);
                    events.signal<InputEvent>().emit(
                        {.key = key, .is_down = event.key.down, .is_repeat = event.key.repeat != 0});
                    break;
                }
                case SDL_EVENT_MOUSE_MOTION:
                    internal::feed_mouse_move(
                        input,
                ...
                {
                    const MouseButton button = internal::translate_mouse_button(event.button.button);
                    if (button != MouseButton::COUNT)
                        internal::feed_mouse_button(input, button, event.button.down);
                    break;
                }
                case SDL_EVENT_MOUSE_WHEEL:
                    internal::feed_scroll(input, event.wheel.y);
                    break;
                case SDL_EVENT_GAMEPAD_ADDED:
                case SDL_EVENT_GAMEPAD_REMOVED:
                case SDL_EVENT_GAMEPAD_BUTTON_DOWN:
                case SDL_EVENT_GAMEPAD_BUTTON_UP:
                case SDL_EVENT_GAMEPAD_AXIS_MOTION:
                    internal::feed_gamepad_event(input, events, event);
                    break;
                default:
                    break;
            }
