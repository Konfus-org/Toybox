...
namespace tbx
{
    ...
    {
        return input.keys[internal::index_of(key)];
    }

    bool is_down(const InputState& input, const MouseButton button)
    {
        return input.mouse[internal::index_of(button)];
    }

    bool is_down(const InputState& input, const GamepadButton button, const int slot)
    {
        return internal::is_valid_gamepad(slot)
            && input.gamepads[static_cast<size>(slot)].buttons[internal::index_of(button)];
    }

    bool is_pressed(const InputState& input, const Key key)
    {
        return input.keys[internal::index_of(key)] && !input.previous_keys[internal::index_of(key)];
    }

    bool is_pressed(const InputState& input, const MouseButton button)
    {
        return input.mouse[internal::index_of(button)] && !input.previous_mouse[internal::index_of(button)];
    }

    bool is_pressed(const InputState& input, const GamepadButton button, const int slot)
    {
        if (!internal::is_valid_gamepad(slot))
            return false;
        const GamepadState& gamepad = input.gamepads[static_cast<size>(slot)];
        return gamepad.buttons[internal::index_of(button)] && !gamepad.previous_buttons[internal::index_of(button)];
    }
    ...
    {
        return internal::is_valid_gamepad(slot) && input.gamepads[static_cast<size>(slot)].is_connected;
    }
    ...
    float get_axis(const InputState& input, const GamepadAxis axis, const int slot)
    {
        if (!internal::is_valid_gamepad(slot))
            return 0.0f;
        return input.gamepads[static_cast<size>(slot)].axes[internal::index_of(axis)];
    }
