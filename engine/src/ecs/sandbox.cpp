...
namespace tbx
{
    ...
    {
        return _sandbox.has_value() && _sandbox->get()._registry.valid(_id);
    }

    bool Toy::has_sticker(std::string_view name) const
    {
        const auto* stickers = _sandbox->get()._registry.try_get<StickerSet>(_id);
        if (!stickers)
            return false;
    ...
    void Toy::remove_sticker(std::string_view name)
    {
        auto* stickers = _sandbox->get()._registry.try_get<StickerSet>(_id);
        if (!stickers)
            return;
        const uint64 wanted = hash_name(name);
        std::erase_if(
    ...
    {
        auto& stickers = _sandbox->get()._registry.get_or_emplace<StickerSet>(_id);
        if (!has_sticker(name))
