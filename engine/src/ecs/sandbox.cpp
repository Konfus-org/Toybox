...
namespace tbx
{
    ...
    namespace internal
    {
        Result<void> instantiate_under(Sandbox& sandbox, AssetsState& assets, EventsState& events, Toy parent, const Kit& kit, std::vector<uint64>& reference_stack, std::vector<ToyId>& spawned);
    ...
    Result<void> internal::instantiate_under(
        Sandbox& sandbox,
        AssetsState& assets,
        EventsState& events,
        Toy parent,
        ...
        std::vector<ToyId>& spawned)
    {
        ...
        for (const Toy& toy : copied)
        {
            ...
            {
                internal::register_streamed_kit(sandbox, assets, events, instance, kit_instance->kit);
                continue;
            }

            const uint64 reference = internal::handle_hash(kit_instance->kit);
            ...
            auto expanded = internal::instantiate_under(
                sandbox,
                assets,
                events,
                instance,
        ...
        const std::span<const Frustum> frustums)
    {
        ...
        internal::open_pending(sandbox, assets, events);
        ...
        for (size i = 0; i < sandbox.streamed_kits.size(); ++i)
        {
            ...
            for (const Frustum& frustum : frustums)
            {
                is_wanted =
                    is_wanted
                    || intersects(frustum, sphere_center, entry.bounds_radius + internal::STREAM_LOAD_MARGIN);
                is_in_sight = is_in_sight
                              || intersects(
                                  frustum,
                                  sphere_center,
                                  entry.bounds_radius + internal::STREAM_UNLOAD_MARGIN);
            }
            ...
            if (!entry.is_loaded && !entry.is_loading && is_wanted)
            {
                ...
                start_detached(
                       ...
                       AssetHandle<Kit> kit) -> Task<void>
                    {
                        ...
                        if (auto expanded = internal::instantiate_under(
                                sandbox,
                                assets,
                                events,
                                instance,
        ...
        WindowsState& windows)
    {
        ...
        // streaming/rendering.
        if (const auto camera_position = internal::primary_camera_position(sandbox))
            internal::update_billboards(sandbox, *camera_position);

        auto frustums = internal::gather_camera_frustums(windows, sandbox);
    ...
    Result<Toy> Sandbox::add(const AssetHandle<Kit>& kit, const Vec3& position)
    {
        ...
        return internal::instantiate_kit(*this, *assets, *events, kit, position);
    }
