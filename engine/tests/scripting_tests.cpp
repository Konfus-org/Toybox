...
function update(toy, delta_time)
    ...
    TEST(Scripts, StartRunsOnceAndUpdateMovesToy)
    {
        ...
        ecs::Toy toy = sandbox.spawn("Grunt").with(Script {.source = *mover});
    ...
    TEST(Scripts, LoadRejectsBadSyntax)
    {
        ...
        // Act
        const auto result = scripts::load_source(runtime.scripts, "broken", "this is not luau ((");
    ...
    TEST(Scripts, ReloadRestartsInstancesAndFiresEvent)
    {
        ...
        const auto reloaded = scripts::reload_source(runtime.scripts, "mover", R"(
function start(toy)
    ...
    TEST(Scripts, ReloadWithBadSyntaxKeepsOldBehavior)
    {
        ...
        // Act
        const auto reloaded = scripts::reload_source(runtime.scripts, "mover", "broken ((");
        scripts::update(runtime.scripts, sandbox, runtime.assets, runtime.events, 0.016f);
...
function fixed_update(toy, delta_time)
    ...
    toy.Transform.position = { x = position.x + 1.0, y = position.y, z = position.z }
        ...
        ecs::Toy toy = sandbox.spawn("Stepper").with(Script {.source = *stepper});
        ...
        scripts::fixed_update(runtime.scripts, 1.0f / 60.0f);
        scripts::fixed_update(runtime.scripts, 1.0f / 60.0f);
        scripts::fixed_update(runtime.scripts, 1.0f / 60.0f);
...
function start(toy)
    ...
    toy.RigidBody = { mass = 5.0, is_kinematic = true }
        ...
        ecs::Toy toy = sandbox.spawn("Buildable").with(Script {.source = *builder});
...
function start(toy)
    ...
    toy.Transform.position = { x = moved.x + yawed.x, y = moved.y, z = moved.z }
        ...
        ecs::Toy toy = sandbox.spawn("Mathy").with(Script {.source = *mathy});
...
function start(toy)
    ...
    toy.Transform.position = {
        ...
        ecs::Toy toy = sandbox.spawn("Typist").with(Script {.source = *typed});
