...
namespace tbx::tests::ecs
{
    ...
    TBX_REGISTER_SERIALIZABLE_STRUCT(TestComponent, id, value)
    ...
    TEST(ECSTests, EntitySerialization_RoundtripsMetadataAndRegisteredComponents)
    {
        ...
        // Act
        const auto json = Json::parse(Serializer<Entity>::to_json(entity));
        auto roundtripped = Entity();
        const bool deserialized = Serializer<Entity>::from_json(json.to_string(-1), roundtripped);
    ...
    TEST(ECSTests, EntitySerialization_SkipsUnregisteredComponents)
    {
        ...
        const bool has_components = json.try_get_child("components", components);

        // Assert
        EXPECT_TRUE(has_components);
        EXPECT_TRUE(components.keys().empty());
    }
