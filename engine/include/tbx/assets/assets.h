...
namespace tbx
{
    ...
    struct TBX_API ResolvedAssetHandle
    {
        Uuid id = {};
    ...
    TBX_API size get_loaded_asset_count(const AssetsState& state);
    ...
    TBX_API std::optional<std::reference_wrapper<std::any>> find_asset(AssetsState& state, const Uuid& id);

    /// @brief
    /// Purpose: Resolves a handle's identity: by id via the meta index, or by path (minting a
    /// .meta sidecar on first sight).
    TBX_API Result<ResolvedAssetHandle> resolve_handle(
        AssetsState& state,
        const Uuid& id,
