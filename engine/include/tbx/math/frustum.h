...
namespace tbx
{
    ...
    TBX_DLL_EXPORT Frustum make_frustum(const Mat4& view_projection);

    /// @brief
    /// Purpose: Whether a bounds sphere touches the frustum (inflate the radius to add a
    /// streaming margin).
    TBX_DLL_EXPORT bool intersects(const Frustum& frustum, const Vec3& sphere_center, float sphere_radius);
}
