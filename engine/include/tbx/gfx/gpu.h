...
namespace tbx
{
    ...
    /// resize.
    struct TBX_DLL_EXPORT FrameDescription
    {
    ...
    TBX_DLL_EXPORT void begin_render_pass(const RenderPassDescription& description);

    /// @brief
    /// Purpose: Ends the current render pass and returns to the swapchain.
    TBX_DLL_EXPORT void end_render_pass();

    /// @brief
    /// Purpose: Compiles and links a shader module from backend-native source (GLSL for gl/).
    TBX_DLL_EXPORT Result<std::unique_ptr<Shader>> compile_shader(
        std::string_view vertex_source,
        std::string_view fragment_source);

    /// @brief
    /// Purpose: Bakes a pipeline-state object (shader + depth/cull/blend).
    TBX_DLL_EXPORT std::unique_ptr<Pipeline> make_render_pipeline(const PipelineDescription& description);

    /// @brief
    /// Purpose: Binds a pipeline; subsequent draw() calls use it.
    TBX_DLL_EXPORT void set_render_pipeline(const Pipeline& pipeline);
    ...
    TBX_DLL_EXPORT void draw(const Mesh& mesh, std::span<const TextureBinding> textures = {});

    /// @brief
    /// Purpose: Current viewport height in pixels.
    TBX_DLL_EXPORT int get_render_viewport_height();

    /// @brief
    /// Purpose: Current viewport width in pixels.
    TBX_DLL_EXPORT int get_render_viewport_width();
    ...
    TBX_DLL_EXPORT bool is_vsync_enabled();
    ...
    TBX_DLL_EXPORT void set_vsync(bool is_enabled);

    /// @brief
    /// Purpose: Reads back one pixel from the current framebuffer — verification/tooling.
    TBX_DLL_EXPORT Color read_pixel_from_frame_buffer(int x, int y);
    ...
    TBX_DLL_EXPORT Result<void> render_screenshot(Texture& result);
    ...
    TBX_DLL_EXPORT Color get_render_clear_color();

    /// @brief
    /// Purpose: Enables/positions the scissor rectangle for UI clipping (y-down coordinates).
    TBX_DLL_EXPORT void set_render_scissor(bool is_enabled, int x, int y, int width, int height);

    /// @brief
    /// Purpose: Sets the drawable region in pixels.
    TBX_DLL_EXPORT void set_render_viewport(int width, int height);
    ...
    TBX_DLL_EXPORT void set_render_viewport(int x, int y, int width, int height);

    /// @brief
    /// Purpose: Creates a square depth-only render target for shadow maps.
    TBX_DLL_EXPORT std::unique_ptr<DepthTarget> make_depth_render_target(int resolution);

    /// @brief
    /// Purpose: Creates an offscreen color+depth render target.
    TBX_DLL_EXPORT std::unique_ptr<RenderTarget> make_render_target(int width, int height);
    ...
    TBX_DLL_EXPORT void apply_shader_uniforms(const Shader& shader, const Json& values);

    /// @brief
    /// Purpose: Reflects a compiled shader's uniform schema (implemented per backend).
    TBX_DLL_EXPORT ShaderInfo reflect_shader(const Shader& shader);
    ...
    TBX_DLL_EXPORT void set_shader_uniform(const Shader& shader, const char* name, const Mat4& value);
    TBX_DLL_EXPORT void set_shader_uniform(const Shader& shader, const char* name, const Vec2& value);
    TBX_DLL_EXPORT void set_shader_uniform(const Shader& shader, const char* name, const Vec3& value);
    TBX_DLL_EXPORT void set_shader_uniform(const Shader& shader, const char* name, const Vec4& value);
    TBX_DLL_EXPORT void set_shader_uniform(const Shader& shader, const char* name, const Color& value);
    TBX_DLL_EXPORT void set_shader_uniform(const Shader& shader, const char* name, float value);
    TBX_DLL_EXPORT void set_shader_uniform(const Shader& shader, const char* name, int value);

    /// @brief
    /// Purpose: Uploads interleaved vertex data; attribute_sizes lists float counts per
    /// attribute (e.g. {3, 4} = position vec3 + color vec4).
    TBX_DLL_EXPORT std::unique_ptr<Mesh> upload_mesh_to_gpu(
        std::span<const float> vertices,
        std::span<const int> attribute_sizes);

    /// @brief
    /// Purpose: Uploads an RGBA8 texture.
    TBX_DLL_EXPORT std::unique_ptr<Texture2d> upload_texture_to_gpu(
        int width,
        int height,
