...
namespace tbx
{
    ...
    TBX_DLL_EXPORT Result<std::vector<std::byte>> read_bytes(const std::filesystem::path& path);

    /// @brief
    /// Purpose: Reads a whole file as text (no encoding conversion).
    TBX_DLL_EXPORT Result<std::string> read_text(const std::filesystem::path& path);

    /// @brief
    /// Purpose: Writes raw bytes to a file, creating parent directories as needed.
    TBX_DLL_EXPORT Result<void> write_bytes(
        const std::filesystem::path& path,
        std::span<const std::byte> bytes);

    /// @brief
    /// Purpose: Writes text to a file, creating parent directories as needed.
    TBX_DLL_EXPORT Result<void> write_text(const std::filesystem::path& path, std::string_view text);
}
