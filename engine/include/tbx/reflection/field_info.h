...
namespace tbx::reflection
{
    ...
    enum class FieldKind : uint8
    {
        ...
        ASSET, // an assets::AssetHandle<T> — serialized as its uuid string
        ASSET_LIST, // a std::vector<assets::AssetHandle<T>> — serialized as an array of uuid strings
    ...
    struct TBX_API FieldInfo
    {
        ...
        // ASSET accessors: the concrete assets::AssetHandle<T> is erased through these so readers
        ...
        // ASSET_LIST accessors: the concrete std::vector<assets::AssetHandle<T>> is erased through
