...
namespace tbx
{
    ...
    TBX_API void gpu_begin_frame(const FrameDescription& description = {});
    ...
    TBX_API void gpu_begin_render_pass(const RenderPassDescription& description);

    /// @brief
    /// Purpose: Ends the current render pass and returns to the swapchain.
    TBX_API void gpu_end_render_pass();

    /// @brief
    /// Purpose: Compiles and links a shader module from backend-native source (GLSL for gl/).
    TBX_API Result<std::unique_ptr<Shader>> gpu_compile_shader(
        std::string_view vertex_source,
        std::string_view fragment_source);

    /// @brief
    /// Purpose: Bakes a pipeline-state object (shader + depth/cull/blend).
    TBX_API std::unique_ptr<Pipeline> gpu_make_pipeline(const PipelineDescription& description);

    /// @brief
    /// Purpose: Binds a pipeline; subsequent gpu_draw() calls use it.
    TBX_API void gpu_set_pipeline(const Pipeline& pipeline);
    ...
    TBX_API void gpu_draw(const Mesh& mesh, std::span<const TextureBinding> textures = {});

    /// @brief
    /// Purpose: Current viewport height in pixels.
    TBX_API int gpu_get_viewport_height();

    /// @brief
    /// Purpose: Current viewport width in pixels.
    TBX_API int gpu_get_viewport_width();
    ...
    TBX_API bool gpu_is_vsync_enabled();
    ...
    TBX_API void gpu_set_vsync(bool is_enabled);

    /// @brief
    /// Purpose: Reads back one pixel from the current framebuffer — verification/tooling.
    TBX_API Color gpu_read_pixel(int x, int y);
    ...
    TBX_API void gpu_set_viewport(int x, int y, int width, int height);

    /// @brief
    /// Purpose: Creates a square depth-only render target for shadow maps.
    TBX_API std::unique_ptr<DepthTarget> gpu_make_depth_target(int resolution);

    /// @brief
    /// Purpose: Creates an offscreen color+depth render target.
    TBX_API std::unique_ptr<RenderTarget> gpu_make_render_target(int width, int height);
    ...
    TBX_API void gpu_apply_uniforms(const Shader& shader, const Json& values);

    /// @brief
    /// Purpose: Reflects a compiled shader's uniform schema (implemented per backend).
    TBX_API ShaderInfo gpu_reflect(const Shader& shader);
    ...
    TBX_API void gpu_set_uniform(const Shader& shader, const char* name, const Mat4& value);
    TBX_API void gpu_set_uniform(const Shader& shader, const char* name, const Vec2& value);
    TBX_API void gpu_set_uniform(const Shader& shader, const char* name, const Vec3& value);
    TBX_API void gpu_set_uniform(const Shader& shader, const char* name, const Vec4& value);
    TBX_API void gpu_set_uniform(const Shader& shader, const char* name, const Color& value);
    TBX_API void gpu_set_uniform(const Shader& shader, const char* name, float value);
    TBX_API void gpu_set_uniform(const Shader& shader, const char* name, int value);

    /// @brief
    /// Purpose: Uploads interleaved vertex data; attribute_sizes lists float counts per
    /// attribute (e.g. {3, 4} = position vec3 + color vec4).
    TBX_API std::unique_ptr<Mesh> gpu_upload_mesh(
        std::span<const float> vertices,
        std::span<const int> attribute_sizes);

    /// @brief
    /// Purpose: Uploads an RGBA8 texture.
    TBX_API std::unique_ptr<Texture2d> gpu_upload_texture(
        int width,
        int height,
