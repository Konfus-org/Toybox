...
namespace tbx
{
    ...
    struct TBX_DLL_EXPORT GamepadState
    {
    ...
    struct TBX_DLL_EXPORT InputState
    {
    ...
    TBX_DLL_EXPORT CursorMode get_cursor_mode(const InputState& input);
    ...
    TBX_DLL_EXPORT void set_cursor_mode(InputState& input, CursorMode mode);
    ...
    TBX_DLL_EXPORT bool is_gamepad_connected(const InputState& input, int slot);

    /// @brief
    /// Purpose: True while the key / mouse button / gamepad button is held.
    TBX_DLL_EXPORT bool is_down(const InputState& input, Key key);
    TBX_DLL_EXPORT bool is_down(const InputState& input, MouseButton button);
    TBX_DLL_EXPORT bool is_down(const InputState& input, GamepadButton button, int slot = 0);

    /// @brief
    /// Purpose: True only on the frame the key / mouse button / gamepad button went down.
    TBX_DLL_EXPORT bool is_pressed(const InputState& input, Key key);
    TBX_DLL_EXPORT bool is_pressed(const InputState& input, MouseButton button);
    TBX_DLL_EXPORT bool is_pressed(const InputState& input, GamepadButton button, int slot = 0);

    /// @brief
    /// Purpose: True only on the frame the key / mouse button / gamepad button went up.
    TBX_DLL_EXPORT bool is_released(const InputState& input, Key key);
    TBX_DLL_EXPORT bool is_released(const InputState& input, MouseButton button);
    TBX_DLL_EXPORT bool is_released(const InputState& input, GamepadButton button, int slot = 0);
