...
namespace Tbx::Tests::Core::Math
{
    ...
    {
        // Arrange
        Vector3 euler(90, 0.0f, 0.0f);

        // Act
        Quaternion q = euler;

        // Assert
        EXPECT_NEAR(q.X, sinf(euler.X / 2.0f), 1e-6f);
        EXPECT_NEAR(q.Y, 0.0f, 1e-6f);
        EXPECT_NEAR(q.Z, 0.0f, 1e-6f);
        EXPECT_NEAR(q.W, cosf(euler.X / 2.0f), 1e-6f);
    }

    TEST(QuaternionTests, FromEulerConstructor_CreatesQuaternionFromEulerAnglesY)
    {
        // Arrange
        Vector3 euler(0.0f, Tbx::Math::PI / 2.0f, 0.0f); // 90 degrees around Y
        ...
        // Assert
        EXPECT_NEAR(q.X, 0.0f, 1e-6f);
        EXPECT_NEAR(q.Y, sinf(euler.Y / 2.0f), 1e-6f);
        EXPECT_NEAR(q.Z, 0.0f, 1e-6f);
        EXPECT_NEAR(q.W, cosf(euler.Y / 2.0f), 1e-6f);
    }

    TEST(QuaternionTests, FromEulerConstructor_CreatesQuaternionFromEulerAnglesZ)
    {
        // Arrange
        Vector3 euler(0.0f, 0.0f, Tbx::Math::PI / 2.0f); // 90 degrees around Z
        ...
        EXPECT_NEAR(q.X, 0.0f, 1e-6f);
        EXPECT_NEAR(q.Y, 0.0f, 1e-6f);
        EXPECT_NEAR(q.Z, sinf(euler.Z / 2.0f), 1e-6f);
        EXPECT_NEAR(q.W, cosf(euler.Z / 2.0f), 1e-6f);
    }
    ...
    {
        // Arrange
        Quaternion q1 = Quaternion::FromEuler(Vector3(Tbx::Math::PI / 4.0f, 0.0f, 0.0f)); // 45 deg X
        Quaternion q2 = Quaternion::FromEuler(Vector3(0.0f, Tbx::Math::PI / 4.0f, 0.0f)); // 45 deg Y
    ...
    {
        // Arrange
        Quaternion rotation = Quaternion::FromEuler(Vector3(0.0f, Tbx::Math::PI / 2.0f, 0.0f)); // 90 deg Y
    ...
    TEST(QuaternionTests, MultiplyStaticMethodQuaternion_MultipliesCorrectly)
    {
        ...
        Quaternion expected(0.3826834f, 0.3826834f, 0.0f, 0.9238795f); // Recalculated
