...
layout(location = 1) in flat float v_shadow_fade;

layout(location = 0) out vec4 o_color;

void main()
{
    // Screen-door fade: thin the colored caster out as its per-instance shadow fade drops below 1,
    // matching the opaque depth pass so a glass caster's tint fades together with its silhouette.
