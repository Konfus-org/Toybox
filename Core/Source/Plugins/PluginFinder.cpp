...
namespace Tbx
{
    static PluginMeta LoadPluginMeta(const std::filesystem::path& pathToMeta)
    {
        ...
        meta.Path = pathToMeta.parent_path().string() + "/" + pathToMeta.filename().stem().string()
                    + ".dll";
#else
        meta.Path =
            pathToMeta.parent_path().string() + "/" + pathToMeta.filename().stem().string() + ".so";
#endif
    ...
    static std::vector<PluginMeta> OrderPluginMetas(const std::vector<PluginMeta>& metas)
    {
        ...
        std::function<void(const PluginMeta&)> visit = [&](const PluginMeta& meta)
        {
            ...
            if (!visiting.insert(meta.Name).second)
            {
                TBX_ASSERT(
                    false,
                    "PluginFinder: Detected cyclic dependency involving '{}'.",
                    meta.Name);
                return;
            }
            ...
            [&](const std::string& name, const std::string& parent)
        {
            ...
            if (it == discoveredByName.end())
            {
                if (parent.empty())
                {
                    TBX_ASSERT(
                        false,
                        "PluginFinder: Requested plugin '{}' was not discovered.",
                        name);
                }
                else
                {
    ...
    std::vector<PluginMeta> PluginFinder::Discover(const std::string& searchDirectory) const
    {
        ...
        {
            TBX_TRACE_ERROR(
                "PluginFinder: Plugin search directory '{}' does not exist.",
                searchDirectory);
                 ...
                 std::filesystem::directory_options::skip_permission_denied))
        {
            ...
            TBX_ASSERT(
                pluginInfoValid,
                "PluginFinder: Invalid plugin info at: {0}!",
                entry.path().string());
            if (!pluginInfoValid)
            {
                continue;
