...
namespace Tbx
{
        ...
        std::vector<Ref<Plugin>>& outLoaded)
    {
        ...
        auto plugin = Ref<Plugin>(
            pluginInstance,
            [unloadPluginFunc, pluginName](Plugin* pluginToUnload)
            {
    ...
    PluginLoader::PluginLoader(const std::vector<PluginMeta>& pluginMetas, Ref<EventBus> eventBus)
        : _eventBus(eventBus)
    {
