...
namespace Tbx
{
        ...
        ExclusiveRef<SharedLibrary>& library)
    {
        ...
        TBX_ASSERT(
            library != nullptr,
            "PluginLibraryManager: Cannot register a plugin without its library.");
        if (!library)
            return;
...

        {
            ...
            {
                TBX_TRACE_WARNING(
                    "PluginLibraryManager: Plugin '{}' registered multiple times; refreshing "
                    "metadata.",
                    meta.Name);
            }
        ...
        if (shouldDispatchLoaded)
            EventCarrier(EventBus::Global).Send(PluginLoadedEvent(weakPlugin, meta));
        else
            TBX_TRACE_VERBOSE(
                "PluginLibraryManager: Duplicate registration detected for '{}'.",
                meta.Name);
    }
    ...
    void PluginManager::HandleDestroyed(PluginDestroyedEvent& event)
    {
        ...
        {
            TBX_TRACE_WARNING(
                "PluginLibraryManager: Received destruction for untracked plugin at {}.",
                static_cast<const void*>(event.Plugin));
            return;
        }
    ...
    void PluginManager::Unregister(const Plugin* plugin)
    {
        ...
        {
            TBX_TRACE_WARNING(
                "PluginLibraryManager: Ignoring manual unregister for null plugin pointer.");
            return;
        }
        ...
        {
            TBX_TRACE_WARNING(
                "PluginLibraryManager: Manual unregister requested for untracked plugin at {}.",
                static_cast<const void*>(plugin));
            return;
        }

        TBX_ASSERT(
            unloaded.Instance.lock().use_count() != 1,
            "PluginLibraryManager: Dangling references detected from manually unregistering plugin "
            "'{}'; .",
            unloaded.Meta.Name);
        EventCarrier(EventBus::Global).Send(PluginUnloadedEvent(unloaded.Instance, unloaded.Meta));
    }
