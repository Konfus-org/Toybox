...
namespace Tbx
{
    ...
    Quaternion Quaternion::FromAxisAngle(const Vector3& axis, float angle)
    {
        ...
        glm::quat result =
            glm::angleAxis(DegreesToRadians(angle), glmAxis); // Convert angle to radians
    ...
    {
        const auto result =
            glm::quat(glm::vec3(DegreesToRadians(x), DegreesToRadians(y), DegreesToRadians(z)));
        return {result.x, result.y, -result.z, result.w};
    }

    Vector3 Quaternion::ToEuler(const Quaternion& quaternion)
    {
        glm::vec3 result = glm::degrees(
            glm::eulerAngles(glm::quat(quaternion.W, quaternion.X, quaternion.Y, quaternion.Z)));
        return {result.x, result.y, result.z};
    }

    bool Quaternion::IsEqualOrEquivalent(
        const Quaternion& lhs,
        const Quaternion& rhs,
        float epsilon)
    {
        // If q and -q are both valid rotations, check both possibilities
        bool directMatch = std::abs(lhs.X - rhs.X) < epsilon && std::abs(lhs.Y - rhs.Y) < epsilon
                           && std::abs(lhs.Z - rhs.Z) < epsilon
                           && std::abs(lhs.W - rhs.W) < epsilon;

        bool negatedMatch = std::abs(lhs.X + rhs.X) < epsilon && std::abs(lhs.Y + rhs.Y) < epsilon
                            && std::abs(lhs.Z + rhs.Z) < epsilon
                            && std::abs(lhs.W + rhs.W) < epsilon;
