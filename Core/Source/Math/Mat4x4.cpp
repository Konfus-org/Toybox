...
namespace Tbx
{
            ...
            for (float val : rowList)
            {
                if (row < 4 && col < 4)
                {
                    ...
                    throw std::out_of_range(
                        "Initializer list has more elements than the destination array.");
            }

            if (col != 4 && row < 4)
                throw std::out_of_range(
                    "Initializer list row has fewer elements than the destination array row.");
            row++;
        }
        if (row != 4)
            throw std::out_of_range("Initializer list has fewer rows than the destination array.");
        ...
        const Vector3& scale)
    {
        ...
        glm::mat4 glmMat = glm::translate(glm::mat4(1.0f), glmPos) * glm::mat4_cast(glmRot)
    ...
    {
        const glm::mat4& glmMat =
            glm::ortho(bounds.Left, bounds.Right, bounds.Bottom, bounds.Top, 0.0f, zFar - zNear);
        return GlmMat4ToTbxMat4x4(glmMat);
    }
    ...
    {
        const glm::mat4 glmMat = glm::make_mat4(matrix.Values.data());
        return GlmMat4ToTbxMat4x4(
            glm::translate(glmMat, glm::vec3(translate.X, translate.Y, translate.Z)));
    }
