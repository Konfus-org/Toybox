...
namespace Tbx
{
        ...
        , _activeGraphicsApi(startingApi)
    {
        TBX_ASSERT(
            !backends.empty(),
            "GraphicsManager: requires at least one graphics backend instance.");
        TBX_ASSERT(eventBus, "GraphicsManager: requires a valid event bus instance.");

        InitializeRenderers(backends, contextProviders);
        TBX_ASSERT(
            !_renderers.empty(),
            "GraphicsManager: No compatible renderer implementations were provided for the "
            "available graphics APIs.");

        _pipeline = GraphicsPipeline(CreateDefaultRenderPasses());
        RecreateRenderersForCurrentApi();

        _eventListener.Listen<AppSettingsChangedEvent>(
            [this](AppSettingsChangedEvent& event)
            {
                OnAppSettingsChanged(event);
            });
        _eventListener.Listen<WindowOpenedEvent>(
            [this](WindowOpenedEvent& event)
            {
                OnWindowOpened(event);
            });
        _eventListener.Listen<WindowClosedEvent>(
            [this](WindowClosedEvent& event)
            {
                OnWindowClosed(event);
            });
        _eventListener.Listen<StageOpenedEvent>(
            [this](StageOpenedEvent& event)
            {
                OnStageOpened(event);
            });
        _eventListener.Listen<StageClosedEvent>(
            [this](StageClosedEvent& event)
            {
                OnStageClosed(event);
            });
    }
    ...
    void GraphicsManager::RecreateRenderersForCurrentApi()
    {
        ...
        for (auto& display : _openDisplays)
        {
            ...
            TBX_ASSERT(
                context,
                "GraphicsManager: Failed to recreate a graphics context for an open window.");
            if (!context)
            {
                continue;
            }
    ...
    void GraphicsManager::OnWindowOpened(const WindowOpenedEvent& e)
    {
        ...
        if (!newWindow)
        {
            TBX_ASSERT(
                false,
                "GraphicsManager: Invalid window open event, ensure a valid window is passed!");
            return;
        }
        ...
        if (!context)
        {
            TBX_ASSERT(
                false,
                "GraphicsManager: Failed to create a graphics context for the opened window.");
            return;
        }
    ...
    void GraphicsManager::OnWindowClosed(const WindowClosedEvent& e)
    {
        ...
        auto displayIt = std::find_if(
            _openDisplays.begin(),
            _openDisplays.end(),
            [closedWindow](const GraphicsDisplay& display)
            {
    ...
    std::vector<RenderPass> GraphicsManager::CreateDefaultRenderPasses()
    {
        ...
        opaque.Draw = [](GraphicsPipeline& pipeline,
        ...
        transparent.Draw = [](GraphicsPipeline& pipeline,
                              GraphicsRenderer& renderer,
                              StageDrawData& renderData,
                              const RenderPass& pass)
        {
