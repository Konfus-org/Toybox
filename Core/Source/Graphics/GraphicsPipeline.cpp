...
namespace Tbx
{
        ...
        float aspectRatio)
    {
        ...
        for (const auto& toy : stageView)
        {
            ...
            if ((hasModel
                 && (hasModelInstance || hasMesh || hasMeshInstance || hasMaterial
                     || hasMaterialInstance))
                || (hasModelInstance
                    && (hasMesh || hasMeshInstance || hasMaterial || hasMaterialInstance))
                || (hasMesh && hasMeshInstance) || (hasMaterial && hasMaterialInstance))
            {
                TBX_ASSERT(
                    false,
                    "GraphicsPipeline: Conflicting model, mesh, or material blocks on toy.");
                continue;
            }
        ...
        const RgbaColor& clearColor)
    {
        TBX_ASSERT(
            &renderer,
            "GraphicsPipeline: No active renderer has been set for the current graphics API.");
        TBX_ASSERT(
            display.Surface && display.Context,
            "GraphicsPipeline: Display is missing a surface or context.");
        ...
        const RenderPass& pass)
    {
        ...
        for (const auto& [shaderProgramId, bucket] : buckets)
        {
            ...
            if (shaderResourceIt == renderer.Cache.ShaderPrograms.end()
                || !shaderResourceIt->second)
            {
                continue;
            }
        ...
        const StageDrawData& renderData)
    {
        for (const auto& toyPtr : toys)
        {
            ...
            for (size_t textureIndex = 0; textureIndex < drawData.Mat->Textures.size();
                 ++textureIndex)
            {
                ...
                if (textureResourceIt == renderer.Cache.Textures.end()
                    || !textureResourceIt->second)
                {
                    continue;
                }
    ...
    bool GraphicsPipeline::ShouldCull(
        const Ref<Toy>& toy,
        ...
        const StageDrawData& renderData)
    {
        ...
        const auto drawDataIt = renderData.Drawables.find(toy->Handle.Id);
        if (drawDataIt == renderData.Drawables.end() || !drawDataIt->second.Mat
    ...
    void GraphicsPipeline::CacheShaders(GraphicsRenderer& renderer, const ShaderProgram& shaders)
    {
        ...
        for (const auto& shader : shaders.Shaders)
        {
            ...
            if (resourceIt == cache.Shaders.end() || !resourceIt->second)
            {
                ...
                TBX_ASSERT(
                    resource,
                    "GraphicsPipeline: Unable to compile shader for active graphics api.");
                if (!resource)
                {
                    continue;
                }
        ...
        TBX_ASSERT(
            shaderProgram,
            "GraphicsPipeline: Unable to cache shader programs for active graphics api.");
        if (!shaderProgram)
        {
            return;
        }
    ...
    void GraphicsPipeline::CacheMaterial(GraphicsRenderer& renderer, const Material& material)
    {
        ...
        for (const auto& texture : material.Textures)
        {
            ...
            TBX_ASSERT(
                resource,
                "GraphicsPipeline: Unable to cache textures for active graphics api.");
            if (!resource)
            {
                continue;
            }
    ...
    size_t GraphicsPipeline::ResolveRenderPassIndex(const Material& material) const
    {
        TBX_ASSERT(
            !RenderPasses.empty(),
            "GraphicsPipeline: Render passes must be configured before drawing.");
