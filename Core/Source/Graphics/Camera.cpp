...
namespace Tbx
{
    ...
    void Camera::SetPerspective(float fov, float aspect, float zNear, float zFar)
    {
        ...
        _fov = fov;
        _projectionMatrix =
            Mat4x4::PerspectiveProjection(DegreesToRadians(fov), aspect, zNear, zFar);
    }
    ...
    Mat4x4 Camera::CalculateViewMatrix(const Vector3& camPosition, const Quaternion& camRotation)
    {
        ...
        Mat4x4 inverseRotationMatrix = Mat4x4::Inverse(
            rotationMatrix); // Or rotationMatrix.Transpose() for orthonormal matrices
    ...
    Mat4x4 Camera::CalculateViewProjectionMatrix(
    ...
    {
        const auto viewProj =
            CalculateViewProjectionMatrix(camPosition, camRotation, projectionMatrix);
