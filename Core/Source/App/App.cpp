...
namespace Tbx
{
        ...
        , _listener(Bus)
    {
        ...
        TBX_ASSERT(
            !_instance,
            "App: Existing singleton was replaced, unexpected behavior may occur.");
        _instance = this;
    }
    ...
    App::~App()
    {
        ...
        if (Status != AppStatus::Closed && Status != AppStatus::Reloading
    ...
    {
        return Status == AppStatus::Running || Status == AppStatus::Paused
    ...
    void App::Run()
    {
        ...
        if (Status != AppStatus::Reloading && Status != AppStatus::Error)
        {
    ...
    void App::Initialize()
    {
        ...
        // 1. Init core systems
        {
            ...
            {
                TBX_TRACE_WARNING(
                    "App: No input handler plugins detected, running headless input.");
                Input = MakeRef<HeadlessInputHandler>();
            }
            else
            {
                if (inputHandlerPlugs.Count() != 1)
                    TBX_TRACE_WARNING(
                        "App: Multiple input handler plugins detected, only one is allowed. Using "
                        "first detected.");
            ...
            {
                if (windowFactories.Count() != 1)
                    TBX_TRACE_WARNING(
                        "App: Multiple window factory plugins detected, only one is allowed. Using "
                        "first detected.");
            ...
            {
                TBX_TRACE_WARNING(
                    "App: Graphics backends and context providers not detected, running headless "
                    "graphics.");
            ...
            {
                if (audioMixers.Count() != 1)
                    TBX_TRACE_WARNING(
                        "App: Multiple audio mixer plugins detected, only one is allowed. Using "
                        "first detected.");
            ...
            {
                TBX_TRACE_WARNING(
                    "App: No asset loader plugins detected, asset server will be non-functional.");
            }
        ...
        _listener.Listen<WindowClosedEvent>(
            [this](WindowClosedEvent& event)
            {
                OnWindowClosed(event);
            });
        _listener.Listen<WindowModeChangedEvent>(
            [this](WindowModeChangedEvent& event)
            {
    ...
    void App::Update()
    {
        ...
        // Debugging and development stuff:
        {
            ...
            {
                // TODO: App slowly eats up more memory every restart meaning we have a leak in our
                // boat! We need to track down why and fix it!
    ...
    {
        // If the window is our main window, set running flag to false which will trigger the app to
        // close
    ...
    void App::DumpFrameReport() const
    {
        ...
        const auto systemTimeMilliseconds = std::chrono::duration_cast<std::chrono::milliseconds>(
                                                systemTimePoint.time_since_epoch())
                                                .count();
        ...
        if (systemTimePoint.time_since_epoch() != Chronometer::SystemClock::duration::zero())
        {
            ...
            const auto systemTimeSubSecond = std::chrono::duration_cast<std::chrono::milliseconds>(
                systemTimePoint.time_since_epoch() % std::chrono::seconds(1));
            systemTimeStream << '.' << std::setw(3) << std::setfill('0')
                             << systemTimeSubSecond.count();
            systemTimeString = systemTimeStream.str();
        }
        constexpr float epsilon = std::numeric_limits<float>::epsilon();
        const float instantaneousFps = deltaSeconds > epsilon ? 1.0f / deltaSeconds : 0.0f;
