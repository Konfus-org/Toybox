...
namespace Tbx
{
    ...
    static void OnAppLaunched(const AppLaunchedEvent& e)
    {
        if (!State.Standby)
        {
            TBX_ASSERT(
                false,
                "Log: Received app launch while logger state was already active, resetting state.");
            ResetState();
        }
        ...
        State.Standby = false;
        if (State.Logger)
            State.Logger->Open(State.Name, "");
        else
            State.Logger = MakeRef<StdOutLogger>();
    ...
    void Log::Trace(LogLevel level, std::string message)
    {
        if (State.Standby)
        {
            if (State.Queue.empty())
            {
                EnsureListenerBound();
                std::printf(
                    "Log: In standby mode, queuing log messages until app launch or manual "
                    "flush.\n\n");
            }
