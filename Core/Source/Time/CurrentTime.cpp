...
namespace Tbx
{
    ...
    SystemTimePoint TruncateTimePoint(
        const SystemTimePoint& timePoint,
        const std::chrono::milliseconds& precision)
    {
        TBX_ASSERT(precision.count() > 0, "Time: Precision must be greater than zero.\n");

        const auto baseDuration = timePoint.time_since_epoch();
        const auto precisionDuration =
            std::chrono::duration_cast<SystemTimePoint::duration>(precision);
