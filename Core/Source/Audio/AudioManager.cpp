...
namespace Tbx
{
        ...
        , _eventListener(eventBus)
    {
        ...
        _eventListener.Listen<AppStatusChangedEvent>(
            [this](const AppStatusChangedEvent& e)
            {
                OnAppStatusChangedEvent(e);
            });
        _eventListener.Listen<StageOpenedEvent>(
            [this](const StageOpenedEvent& e)
            {
                OnStageOpened(e);
            });
        _eventListener.Listen<StageClosedEvent>(
            [this](const StageClosedEvent& e)
            {
                OnStageClosed(e);
            });
    }
