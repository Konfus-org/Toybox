...
namespace Tbx
{
    ...
    class FactoryPlugin
        : public Plugin
        , public std::enable_shared_from_this<FactoryPlugin<TProduct>>
    {
      public:
        ...
        {
            auto product = Ref<TProduct>(
                new TProduct(std::forward<Args>(args)...),
                [&](TProduct* prodToDelete)
                {
                    Destroy(prodToDelete);
                });
