...
namespace Tbx
{
    ...
    /// Once this object goes out of scope the destructor is responsible for cleaning up the
    /// resource.
    /// </summary>
    ...
    /// <summary>
    /// A RAII wrapper for GraphicsResource that automatically calls Activate() and Release() on
    /// construction and destruction.
    /// </summary>
    class TBX_EXPORT UseGraphicsResourceScope
    {
      public:
        UseGraphicsResourceScope(Ref<GraphicsResource> resource)
            : _resource(resource)
        {
            _resource->Activate();
        }
        ~UseGraphicsResourceScope()
        {
            _resource->Release();
        }
