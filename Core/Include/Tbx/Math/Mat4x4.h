...
namespace Tbx
{
    ...
    {
        Mat4x4();
        explicit(false) Mat4x4(const std::array<float, 16>& data)
            : Values(data)
        {
        }
        ...
        friend Mat4x4 operator*(float lhs, const Mat4x4& rhs)
        {
            return Multiply(lhs, rhs);
        }
        friend Mat4x4 operator*(const Mat4x4& lhs, float rhs)
        {
            return Multiply(lhs, rhs);
        }
        ...
        explicit(false) operator std::array<float, 16>() const
        {
            return Values;
        }
