    ...
    struct TBX_EXPORT BlockCollection
    {
      ...
      public:
        ...
        Ref<T> Add(Args&&... args)
        {
            ...
            {
                TBX_TRACE_WARNING(
                    "BlockCollection: pool for %s is exhausted. Call "
                    "BlockCollection::Reserve<%s>(additionalCapacity) before adding more "
                    "instances.",
                    typeId.name(),
                    typeId.name());
                return Ref<T>();
        ...
        /// <summary>
        /// Attempts to retrieve a shared pointer to the stored block of type <typeparamref
        /// name="T"/>.
        /// </summary>
        template <typename T>
