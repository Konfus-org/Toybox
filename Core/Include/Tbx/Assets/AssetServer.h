...
{
    /// <summary>
    /// Central coordinator for discovering, loading, and caching assets backed by registered
    /// loaders.
    /// </summary>
    class TBX_EXPORT AssetServer
    {
      public:
        ...
        void Register(const std::string& path)
        {
            ...
            if (!FindLoaderForType<TData>(absolutePath))
            {
                TBX_ASSERT(
                    false,
                    "AssetServer: Unable to register {} because no loader accepted the file",
                    path);
            }
        }
      ...
      private:
        ...
        template <typename TData>
        Ref<TData> LoadData(const std::string& assetName, const std::filesystem::path& filePath)
            const
        {
            ...
            if (cacheIt != _loadedAssets.end())
            {
                ...
                else
                {
                    ...
                    if (typeIt != _assetTypes.end()
                        && typeIt->second == std::type_index(typeid(TData)))
                    {
            ...
            try
            {
                ...
                {
                    TBX_TRACE_ERROR(
                        "AssetServer: loader returned unexpected type for {}",
                        assetName);
                    return nullptr;
                }
      ...
      private:
        ...
        // TODO: memory pools for each asset type that allocates a target amount of memory and keeps
        // track of the available memory, when its full it'll clear out old stuff AKA things at the
        // beginning of the pool and are not in use (which is known via a shared pointer ref count)
