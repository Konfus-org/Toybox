...
namespace Tbx
{
    ...
    struct MemoryPool
    {
      public:
        /// <summary>
        /// Builds a pool that preallocates <paramref name="capacity"/> slots sized for
            ...
            : _state(MakeRef<MemoryState>())
        {
            TBX_ASSERT(
                capacity > 0,
                "MemoryPool<{}>: capacity must be greater than zero",
                typeid(TObject).name());
            Initialize(capacity);
            TBX_TRACE_INFO(
                "MemoryPool<{}>: allocated {} slots of size {} bytes\n",
                typeid(TObject).name(),
                capacity,
                sizeof(TObject));
        }
        ...
        Ref<TObject> Provide(TArgs&&... args)
        {
            ...
            {
                TBX_TRACE_ERROR(
                    "MemoryPool<{}>: exhausted pool backing {} objects (capacity={}) for the type!",
                    typeid(TObject).name(),
                    sizeof(TObject),
                    Capacity());
                return Ref<TObject>();
            ...
            {
                TBX_ASSERT(
                    slot.Chunk < state->Chunks.size(),
                    "MemoryPool<{}>: chunk index out of bounds!",
                    typeid(TObject).name());
                ...
                TBX_ASSERT(
                    slot.Index < chunk.Capacity,
                    "MemoryPool<{}>: slot index out of bounds!",
                    typeid(TObject).name());
            ...
            auto deleter =
                [state, chunkIndex = slot.Chunk, slotIndex = slot.Index](TObject* pointer)
            {
                if (pointer == nullptr)
                {
                    return;
        ...
        void Reserve(uint64 additionalCapacity)
        {
            TBX_ASSERT(
                ...
                "MemoryPool<{}>: additional capacity must be greater than zero!",
                typeid(TObject).name());
            ...
            TBX_TRACE_INFO(
                "MemoryPool<{}>: reserved {} additional slots (object size={} bytes, new "
                "capacity={})\n",
                typeid(TObject).name(),
                additionalCapacity,
                sizeof(TObject),
      ...
      private:
        ...
        static void Release(const Ref<MemoryState>& state, uint64 chunkIndex, uint64 slotIndex)
        {
            ...
            TBX_ASSERT(
                chunk.States[slotIndex],
                "MemoryPool<{}>: double free detected!",
                typeid(TObject).name());
            ...
            TBX_ASSERT(
                state->Count > 0,
                "MemoryPool<{}>: release called on empty pool!",
                typeid(TObject).name());
            if (state->Count > 0)
            {
                state->Count--;
