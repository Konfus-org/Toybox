...
#include "Tbx/Debug/Log.h"
    ...
    /// the log. Msg will be written to a log file in release and a console in debug. And if in
    /// debug and our assert failed (evaluated to false) the app will break into the debugger This
    /// is a good method to use to validate things.
    /// </summary>
    ...
    #endif
    #define TBX_ASSERT(check, ...)                                                                 \
        do                                                                                         \
        {                                                                                          \
            if (!(check))                                                                          \
            {                                                                                      \
                Tbx::Log::Critical(__VA_ARGS__);                                                   \
                TBX_DEBUG_BREAK();                                                                 \
            }                                                                                      \
        } while (0)
#else
...
/// If a log is created and listening the msg will be written to a log file in release, others a
/// console in debug. And if in debug and our assert failed (evaluated to false) the app will break
/// into the debugger
/// </summary>
...
/// into the debugger
/// </summary>
#define TBX_VALIDATE_WEAK_PTR(ptr, ...)                                                            \
    TBX_ASSERT((!ptr.expired() && ptr.lock() != nullptr), __VA_ARGS__)
