...
namespace Tbx
{
    ...
    void GraphicsPipeline::RenderCameraView(const Tbx::RenderBucket& bucket, const Tbx::CameraData& camera, const Tbx::Ref<Tbx::ShaderProgramResource>& shaderResource, Tbx::GraphicsRenderer& renderer, const StageDrawData& renderData)
    {
        ...
        {
            if (ShouldCull(entityPtr, camera.Frustum, renderData))
            {
                continue;
            }
            ...
            if (!drawData.Material || !drawData.Mesh)
            {
                continue;
            }

            std::vector<UseGraphicsResourceScope> textureScopes = {};
            textureScopes.reserve(drawData.Material->Textures.size());
            for (size_t textureIndex = 0; textureIndex < drawData.Material->Textures.size(); ++textureIndex)
            {
                const auto& texture = drawData.Material->Textures[textureIndex];
                if (!texture)
                {
                    continue;
                }
            ...
            const auto meshResourceIt = renderer.Cache.Meshes.find(drawData.Mesh->Id);
        ...
        float aspectRatio)
    {
        ...
        for (const auto& toy : stageView)
        {
            ...
            if (toy->TryGet(modelRef) && modelRef)
            {
                ...
                meshRegistry[modelRef->Mesh.Id] = &modelRef->Mesh;
                materialRegistry[modelRef->Material.Id] = &modelRef->Material;
            }
        }
        ...
        for (const auto& toy : stageView)
        {
            ...
            if (resolvedModel)
            {
                resolvedMaterial = &resolvedModel->Material;
            }
            else if (hasMaterial)
            {
            ...
            if (resolvedModel)
            {
                resolvedMesh = &resolvedModel->Mesh;
            }
            else if (hasMesh)
            {
            ...
            passBuckets[resolvedMaterial->ShaderProgram.Id].push_back(toy);
    ...
    bool GraphicsPipeline::ShouldCull(const Ref<Toy>& toy, const Frustum& frustum, const StageDrawData& renderData)
    {
        ...
        if (drawDataIt == renderData.Drawables.end() ||
            !drawDataIt->second.Material ||
            !drawDataIt->second.Mesh)
        {
            return true;
        }
