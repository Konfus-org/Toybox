...
namespace Tbx::Plugins::SDL3Audio
{
    ...
    SDL3AudioPlugin::SDL3AudioPlugin(Ref<EventBus> eventBus)
    {
        ...
        {
            TBX_TRACE_ERROR(
                "SDL3Audio: Failed to initialize SDL audio subsystem: {}",
                SDL_GetError());
        }
        ...
        TBX_TRACE_INFO(
            "SDL3Audio: Initialized with device format {}, Hz {}, channels {}",
            _deviceSpec.freq,
            static_cast<int>(_deviceSpec.channels),
            SDL_GetAudioFormatName(_deviceSpec.format));
    }
    ...
    void SDL3AudioPlugin::SetPosition(const Audio& audio, const Vector3& position)
    {
        ...
        {
            TBX_TRACE_WARNING(
                "SDL3Audio: Spatial playback requested for asset {} but it could not be set. Is "
                "the audio device stereo?",
                audio.Id.ToString());
        }
    }
    ...
    Ref<Audio> SDL3AudioPlugin::LoadAudio(const std::filesystem::path& filepath)
    {
        ...
        {
            TBX_TRACE_ERROR(
                "SDL3Audio: Failed to convert audio '{}': {}",
                filepath.string(),
                SDL_GetError());
            return nullptr;
        }
    ...
    bool SDL3AudioPlugin::SetPlaybackParams(
        ...
        const PlaybackParams& params)
    {
        ...
        {
            TBX_TRACE_WARNING(
                "SDL3Audio: Failed to adjust audio stream playback ratio: {}",
                SDL_GetError());
        }
        ...
        {
            TBX_TRACE_WARNING(
                "SDL3Audio: Failed to adjust audio stream volume: {}",
                SDL_GetError());
        }
        ...
        const bool spatialChanged = instance.Spatial
                                    && (previousSpace.Left != instance.SpatialGain.Left
                                        || previousSpace.Right != instance.SpatialGain.Right);
        if (spatialChanged)
        {
            if (!ConfigureChannelMap(instance, instance.SpatialGain))
            {
                TBX_TRACE_WARNING(
                    "SDL3Audio: Unable to refresh spatial channel map: {}",
                    SDL_GetError());
            }
        }
        ...
        if (instance.Loop)
        {
            ...
            {
                TBX_TRACE_WARNING(
                    "SDL3Audio: Failed to query queued audio for asset {}: {}",
                    audio.Id.ToString(),
                    SDL_GetError());
                return true;
            }
    ...
    SpatialSettings SDL3AudioPlugin::ResolveSpatialSettings(
        const Audio& audio,
        const Vector3& position) const
    {
        ...
        const bool formatSupportsSpatial =
            audio.Format.SampleFormat == AudioSampleFormat::Float32 && audio.Format.Channels > 0;
        if (!formatSupportsSpatial)
        {
            TBX_TRACE_WARNING(
                "SDL3Audio: Spatial playback requested for asset {} but unsupported format was "
                "provided.",
                audio.Id.ToString());
            return settings;
        }
        ...
        {
            TBX_TRACE_WARNING(
                "SDL3Audio: Spatial playback requested for asset {} but the audio device is not "
                "stereo.",
                audio.Id.ToString());
            return settings;
        }
    ...
    PlaybackInstance* SDL3AudioPlugin::GetOrCreatePlayback(
        const Audio& audio,
        ...
        bool createIfMissing)
    {
        ...
        if (it != _playbackInstances.end())
        {
            ...
            if (spatial != nullptr)
            {
                const bool layoutMismatch =
                    instance.Stream != nullptr && instance.Spatial != spatial->Enabled;
                if (layoutMismatch)
                {
    ...
    bool SDL3AudioPlugin::BuildPlaybackStream(
        PlaybackInstance& instance,
        const Audio& audio,
        const SpatialSettings& settings)
    {
        ...
        {
            TBX_TRACE_WARNING(
                "SDL3Audio: Audio asset {} contains no playable data.",
                audio.Id.ToString());
            return false;
        }
        ...
        {
            TBX_TRACE_WARNING(
                "SDL3Audio: Unsupported audio sample format for asset {}.",
                audio.Id.ToString());
            return false;
        }
        ...
        if (settings.Enabled)
        {
            ...
            {
                TBX_TRACE_ERROR(
                    "SDL3Audio: Failed to configure spatial channel map: {}",
                    SDL_GetError());
    ...
    bool SDL3AudioPlugin::ConfigureChannelMap(PlaybackInstance& instance, const StereoSpace& stereo)
    {
        ...
        std::vector<int> channelMap(static_cast<size_t>(outputChannels) *
        static_cast<size_t>(inputChannels), 0.0f); const auto index = [inputChannels](int
        destination, int source) -> size_t
            {
                return static_cast<size_t>(destination) * static_cast<size_t>(inputChannels) +
    ...
    bool SDL3AudioPlugin::SubmitAudioData(
        PlaybackInstance& instance,
        const Audio& audio,
        bool resetStream)
    {
        ...
        const auto queueRaw = [&](const void* buffer, size_t size) -> bool
        {
            ...
            {
                TBX_TRACE_ERROR(
                    "SDL3Audio: Audio asset {} is too large to queue for playback.",
                    audio.Id.ToString());
                return false;
            }
        ...
        {
            TBX_TRACE_ERROR(
                "SDL3Audio: Spatial playback requires float32 audio data for asset {}.",
                audio.Id.ToString());
            return false;
        }
        ...
        {
            TBX_TRACE_ERROR(
                "SDL3Audio: Unexpected audio buffer size for asset {}.",
                audio.Id.ToString());
            return false;
        }
        ...
        {
            TBX_TRACE_ERROR(
                "SDL3Audio: Spatial playback could not interpret audio samples for asset {}.",
                audio.Id.ToString());
            return false;
        }
