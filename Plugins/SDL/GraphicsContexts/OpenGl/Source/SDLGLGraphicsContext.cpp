...
namespace Tbx::Plugins::SDLGraphicsContext
{
        ...
        : _window(window)
    {
        ...
        TBX_ASSERT(
            att == SDL_GL_CONTEXT_PROFILE_CORE,
            "SDLGLContext: Failed to set OpenGL context profile to core");
#ifdef TBX_DEBUG
        SDL_GL_GetAttribute(SDL_GL_CONTEXT_FLAGS, &att);
        TBX_ASSERT(
            att == SDL_GL_CONTEXT_DEBUG_FLAG,
            "SDLGLContext: Failed to set OpenGL context debug flag");
#endif
