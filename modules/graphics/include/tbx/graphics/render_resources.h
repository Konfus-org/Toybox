...
namespace tbx
{
    ...
    class TBX_API RenderResourceManager
    {
      ...
      public:
        Uuid add_dynamic_mesh(const DynamicMesh& dynamic_mesh, bool pin = false);
        Uuid add_static_mesh(const StaticMesh& static_mesh, bool pin = false);
        Uuid add_material(const MaterialInstance& material_instance, bool pin = false);
        std::shared_ptr<Material> get_material_asset(const Handle& material_handle);
        Uuid add_texture(const Texture& texture, const Uuid& resource_uuid, bool pin = false);
        Uuid add_render_texture(
            const TextureSettings& texture_settings,
            const Uuid& resource_uuid = {},
            bool pin = false);
        Uuid add_texture(const Handle& texture_handle, bool pin = false);
        void destroy(const Uuid& resource_uuid);
