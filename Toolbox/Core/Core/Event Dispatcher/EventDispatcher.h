...
#include "Core/Callbacks/CallbackAPI.h"
#include <typeindex>
#include <mutex>
...
namespace Tbx
{
    ...
    public:
        ...
        TBX_API static inline UID Subscribe(const CallbackFunction<TEvent>& callback)
        {
        ...
        TBX_API static inline void Unsubscribe(const UID& callbackToUnsub)
        {
            std::scoped_lock<std::mutex> lock(GetMutex());
