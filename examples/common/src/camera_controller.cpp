...
{
    void CameraController::initialize(Entity camera, const CameraControllerSettings& settings)
    {
        clear_input_context();
        ...
        const CameraControllerSettings& settings)
    {
        ...
        auto scheme_name = std::string("FreeFlyCamera");
    ...
    void CameraController::shutdown(InputManager& input_manager)
    {
    ...
    InputScheme& CameraController::get_input_scheme()
    {
        return get_registered_input_scheme();
    }

    void CameraController::update(const DeltaTime& dt)
    {
        ...
        {
            auto vertical_move = Vec3(_up_down_axis.x, _up_down_axis.y, 0.0F) * _move_speed
                                 * static_cast<float>(dt.seconds);
            camera_transform.position += vertical_move;
        }

        camera_transform.rotation = normalize(yaw_rotation * pitch_rotation);
    }

    const Entity& CameraController::get_camera() const
    {
        return _camera_entity;
    }

    InputAction CameraController::create_move_action()
    {
        return InputAction(
            "Move",
    ...
    InputAction CameraController::create_look_action()
    {
        return InputAction(
            "Look",
            InputActionValueType::VECTOR2,
    ...
    InputAction CameraController::create_up_down_action()
    {
        return InputAction(
            "UpDown",
            InputActionValueType::VECTOR2,
    ...
    Vec3 CameraController::normalize_or_zero(const Vec3& value)
    {
