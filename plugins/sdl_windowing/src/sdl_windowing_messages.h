...
namespace tbx::plugins
{
    ...
    struct TBX_API WindowMakeCurrentRequest : public Request<void>
    {
        ...
        /// Thread Safety: Safe to construct on any thread; handling is render-thread only.</remarks>
        explicit WindowMakeCurrentRequest(const Uuid& window_id)
            : window(window_id)
        {
    ...
    struct TBX_API WindowPresentRequest : public Request<void>
    {
        ...
        /// Ownership: Copies the identifier value.
        /// Thread Safety: Safe to construct on any thread; handling is render-thread only.</remarks>
        explicit WindowPresentRequest(const Uuid& window_id)
            : window(window_id)
        {
            require_handling = true;
        }
