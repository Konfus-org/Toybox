...
namespace tbx::windows_rpc
{
    ...
    /// tbx::IRpcRouter so any plugin can register methods; the WindowsRPC plugin drives invoke() from
    /// its update loop and owns the lone strong reference (via the service provider).
    /// @details
