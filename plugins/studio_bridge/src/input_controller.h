...
namespace tbx::studio_bridge
{
    ...
    /// @details
    /// Ownership: Owns only the last-reported mouse-lock mode; holds non-owning references to the
    /// engine services (incl. the RPC host) and the view manager. Thread Safety: Main-thread only
    /// (driven from on_update).
