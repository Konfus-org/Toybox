...
namespace tbx::studio_bridge
{
    ...
    // each verb's address (describe/set) and routes it to the implementation that owns what the address
    // names — entity/component/world ops in world_ops, asset describes in asset_ops. These functions own
    // the address grammar; the targets own the behavior. Main-thread only (request handling).
