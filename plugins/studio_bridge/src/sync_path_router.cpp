...
namespace tbx::studio_bridge
{
    ...
    enum class PathKind
    {
        ...
        AssetDescribe,     // stream/{destination}/{assetId}
    ...
    SyncPathRouter::SyncPathRouter(WorldManager& world, AssetOps& assets)
        : _world(world)
        , _assets(assets)
    {
    }

    Result SyncPathRouter::sync_describe_path(const tbx::Json& params, tbx::Json& out_reply) const
    {
        ...
        switch (parse_sync_path(path, legacy, scalar))
        {
            ...
            case PathKind::EntityScalar:
            {
                ...
                if (const auto result = _world.get().describe_entity(legacy, reply); !result)
                    return result;
            ...
            case PathKind::AssetDescribe:
                return _assets.get().describe_asset(legacy, out_reply);
            case PathKind::SettingsDescribe:
                out_reply = _world.get().describe_settings();
                return Result::OK;
            case PathKind::WorldDescribe:
                out_reply = _world.get().describe_world(legacy);
                return Result::OK;
            default:
    ...
    Result SyncPathRouter::sync_set_path(const tbx::Json& params) const
    {
        const auto path = params.value(Wire::PATH, std::string());
        ...
        switch (parse_sync_path(path, legacy, scalar))
        {
            case PathKind::ComponentProperty:
                ...
                return sync_set(legacy);
            case PathKind::EntityScalar:
                return _world.get().set_entity_scalar(legacy, scalar, *value_iterator);
            default:
                return Result(false, "sync.set: unsupported path '" + path + "'.");
        }
    }

    Result SyncPathRouter::sync_reset_path(const tbx::Json& params) const
    {
        const auto path = params.value(Wire::PATH, std::string());
        auto legacy = tbx::Json::object();
        auto scalar = std::string();
    ...
    Result SyncPathRouter::sync_is_default_path(const tbx::Json& params, bool& out_is_default) const
    {
        const auto path = params.value(Wire::PATH, std::string());
        auto legacy = tbx::Json::object();
        auto scalar = std::string();
        ...
        {
            case PathKind::ComponentProperty:
                return sync_is_default(legacy, out_is_default);
            default:
    ...
    Result SyncPathRouter::sync_set(const tbx::Json& params) const
    {
        auto entity = tbx::Entity();
        if (const auto resolved = _world.get().resolve_sync_entity(params, entity); !resolved)
            return resolved;
        ...
        const auto property = params.value(Wire::PROPERTY, std::string());
        if (property.empty())
    ...
    Result SyncPathRouter::sync_reset(const tbx::Json& params) const
    {
        auto entity = tbx::Entity();
        if (const auto resolved = _world.get().resolve_sync_entity(params, entity); !resolved)
            return resolved;
    ...
    Result SyncPathRouter::sync_is_default(const tbx::Json& params, bool& out_is_default) const
    {
        auto entity = tbx::Entity();
        if (const auto resolved = _world.get().resolve_sync_entity(params, entity); !resolved)
            return resolved;
