...
namespace tbx::studio_bridge
{
    ...
    /// @details
    /// Ownership: Owns its registration with the engine log (a listener id). Borrows the engine
    /// services (for the RPC host). Thread Safety: The forwarding listener runs on whatever thread
    /// logs; the rest are main-thread.
    class LogBridge
    {
      public:
        ...
        // Writes an editor-originated line into the engine's unified log (no RPC echo back).
        void write_editor_log(const tbx::Json& params);

        // Applies the editor's per-level log colours (info / warning / error+critical).
