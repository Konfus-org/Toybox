...
namespace tbx::studio_bridge
{
                ...
                for (auto& view_ptr : view_streams)
                {
                    ...
                    if (auto* preview = dynamic_cast<AssetPreviewViewStream*>(view_ptr.get()))
                    {
                        if (input.focused)
                        {
                            ...
                            if ((input.buttons & (ViewButtons::LEFT | ViewButtons::RIGHT)) != 0U)
                            {
                                preview->orbit_yaw -=
                    ...
                    if ((input.buttons & ViewButtons::RIGHT) != 0U
                        && (input.accumulated_mouse_dx != 0.0F
                            || input.accumulated_mouse_dy != 0.0F))
                    {
                        auto rotated =
                    ...
                    // Fly only while right-mouse look is held (the Unity model): otherwise the bare
                    // move keys stay free for the editor's tool shortcuts (Q/W/E/R gizmo modes).
                    if ((input.buttons & ViewButtons::RIGHT) != 0U)
                    {
                    ...
                    if ((input.buttons & ViewButtons::MIDDLE) != 0U)
                    {
                        camera.position +=
    ...
    {
        auto input_manager = services.input_manager.lock();
        if (!input_manager)
            return;
                ...
                std::unordered_map<std::string, ViewInput>& inputs)
            {
                for (auto& view_ptr : view_streams)
                {
                    ...
                    if (is_playing && input.focused && !external.enabled)
                    {
                        ...
                        // Studio button bits (ViewButtons) → SDL ids (1 left, 2 middle, 3 right).
                        if ((input.buttons & ViewButtons::LEFT) != 0U)
    ...
    void report_mouse_lock(InputState& input, const EngineServices& services, bool is_playing)
    {
        ...
        if (mode == input.last_reported_lock)
            return;

        input.last_reported_lock = mode;
