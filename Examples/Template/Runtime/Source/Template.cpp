...
void Demo::OnStart(Tbx::App* owner)
{
    ...
    if (owner == nullptr || owner->Assets == nullptr)
    {
        TBX_TRACE_WARNING("Demo: owner or assets missing, unable to initialize scene.\n");
        return;
    }
    ...
    // Setup testing scene:
    {
        ...
        // We need to have at least once instance of our mesh and materials to be able to instance
        // them
        ...
        // Create smily
        {
            ...
            transform.SetPosition({0, _smilyBaseHeight, 100})
    ...
    if (owner->Bus != nullptr)
    {
        Tbx::EventCarrier carrier(owner->Bus);
...
void Demo::OnUpdate(const Tbx::DeltaTime& deltaTime)
{
    ...
    if (input != nullptr)
    {
        ...
        // Camera rotation
        {
            const float camRotateSpeedDegPerSec =
                180.0f; // for keys / gamepad (keep with deltaTime)
            const float mouseSensitivityDegPerPx = 0.35f; // tune to taste
            const float mouseSmoothingRate =
                11.0f; // higher == snappier response, lower == smoother

            // Arrow and gamepad btn style
            {
                if (input->IsKeyHeld(TBX_KEY_LEFT)
                    || input->IsGamepadButtonHeld(0, TBX_GAMEPAD_BUTTON_WEST))
                    _camYaw -= camRotateSpeedDegPerSec * deltaTimeSeconds;
                if (input->IsKeyHeld(TBX_KEY_RIGHT)
                    || input->IsGamepadButtonHeld(0, TBX_GAMEPAD_BUTTON_EAST))
                    _camYaw += camRotateSpeedDegPerSec * deltaTimeSeconds;
                if (input->IsKeyHeld(TBX_KEY_UP)
                    || input->IsGamepadButtonHeld(0, TBX_GAMEPAD_BUTTON_NORTH))
                    _camPitch += camRotateSpeedDegPerSec * deltaTimeSeconds;
                if (input->IsKeyHeld(TBX_KEY_DOWN)
                    || input->IsGamepadButtonHeld(0, TBX_GAMEPAD_BUTTON_SOUTH))
                    _camPitch -= camRotateSpeedDegPerSec * deltaTimeSeconds;
            }
            // Mouse and gamepad axis style
            {
                ...
                {
                    const Tbx::Vector2 targetLookDelta = Tbx::Vector2(
                        mouseDelta.X * mouseSensitivityDegPerPx,
                        mouseDelta.Y * mouseSensitivityDegPerPx);

                    const float smoothingFactor =
                        std::clamp(deltaTimeSeconds * mouseSmoothingRate, 0.0f, 1.0f);
        ...
        if (input->IsKeyHeld(TBX_KEY_LEFT_SHIFT) || input->IsKeyHeld(TBX_KEY_RIGHT_SHIFT)
        ...
        // Get movement dir
        {
            ...
            // Get WASD/dpad style
            {
                if (input->IsKeyHeld(TBX_KEY_W)
                    || input->IsGamepadButtonHeld(0, TBX_GAMEPAD_BUTTON_DPAD_UP))
                    camMoveDir += Tbx::Quaternion::GetForward(camTransform.Rotation);
                if (input->IsKeyHeld(TBX_KEY_S)
                    || input->IsGamepadButtonHeld(0, TBX_GAMEPAD_BUTTON_DPAD_DOWN))
                    camMoveDir -= Tbx::Quaternion::GetForward(camTransform.Rotation);
                if (input->IsKeyHeld(TBX_KEY_D)
                    || input->IsGamepadButtonHeld(0, TBX_GAMEPAD_BUTTON_DPAD_RIGHT))
                    camMoveDir -= Tbx::Quaternion::GetRight(camTransform.Rotation);
                if (input->IsKeyHeld(TBX_KEY_A)
                    || input->IsGamepadButtonHeld(0, TBX_GAMEPAD_BUTTON_DPAD_LEFT))
                    camMoveDir += Tbx::Quaternion::GetRight(camTransform.Rotation);
                if (input->IsKeyHeld(TBX_KEY_E)
                    || input->IsGamepadButtonHeld(0, TBX_GAMEPAD_BUTTON_RIGHT_BUMPER))
                    camMoveDir += Tbx::Vector3::Up;
                if (input->IsKeyHeld(TBX_KEY_Q)
                    || input->IsGamepadButtonHeld(0, TBX_GAMEPAD_BUTTON_LEFT_BUMPER))
