...
namespace tbx
{
    ...
    TBX_API std::string sanitize_string_for_file_name_usage(const std::string_view value);

    // Extracts only the filename component from a path (without directories).
    TBX_API std::string get_filename_from_string_path(const std::string_view path);
}
