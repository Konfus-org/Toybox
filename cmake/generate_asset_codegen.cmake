...
elseif(TBX_ASSET_CODEGEN_MODE STREQUAL "MATERIAL_INSTANCES")
    ...
    foreach(relative_material_path IN LISTS material_files)
        ...
        string(JSON textures_type ERROR_VARIABLE textures_type_error TYPE "${material_text}" textures values)
        if(textures_type_error STREQUAL "NOTFOUND")
            string(JSON texture_count LENGTH "${material_text}" textures values)
            if(texture_count GREATER 0)
                math(EXPR last_texture_index "${texture_count} - 1")
                foreach(texture_index RANGE ${last_texture_index})
                    string(JSON texture_name GET "${material_text}" textures values ${texture_index} name)
        ...
        string(JSON parameters_type ERROR_VARIABLE parameters_type_error TYPE "${material_text}" parameters values)
        if(parameters_type_error STREQUAL "NOTFOUND")
            string(JSON parameter_count LENGTH "${material_text}" parameters values)
            if(parameter_count GREATER 0)
                ...
                foreach(parameter_index RANGE ${last_parameter_index})
                    string(JSON parameter_name GET "${material_text}" parameters values ${parameter_index} name)
                    string(JSON parameter_type ERROR_VARIABLE parameter_type_error GET "${material_text}" parameters values ${parameter_index} type)
                    if(NOT parameter_type_error STREQUAL "NOTFOUND")
                        string(JSON parameter_type GET "${material_text}" parameters values ${parameter_index} data type)
                    endif()
